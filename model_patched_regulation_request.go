...
type PatchedRegulationRequest struct {
	...
	// The subject of the regulation.  * `privacy` - Privacy * `finance` - Finance * `education` - Education * `medical` - Medical * `corporate` - Corporate * `security` - Security * `government` - Government * `other` - Other
...
type _PatchedRegulationRequest PatchedRegulationRequest
...
func (o *PatchedRegulationRequest) UnmarshalJSON(data []byte) (err error) {
	varPatchedRegulationRequest := _PatchedRegulationRequest{}

	err = json.Unmarshal(data, &varPatchedRegulationRequest)

	if err != nil {
		return err
	}

	*o = PatchedRegulationRequest(varPatchedRegulationRequest)
